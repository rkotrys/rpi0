...
class clock:
    ...
    def runclock(self):
        ...
        draw.text( ((128-self.font.getsize('40')[0])/2,85), self.msg, font=self.font, fill=iconcolor )
        hostname = str(proc.check_output(['hostname'] ), encoding='utf-8').strip()
        draw.text( ((128-self.font12.getsize(hostname)[0])/2,70), hostname, font=self.font12, fill=iconcolor )
        symbol = ""
        if wlanip != "" and ethip != "":
