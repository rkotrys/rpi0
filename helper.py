...
def getbluetooth_info():
    ...
    out = str( subprocess.run([ '/bin/bluetoothctl show'  ], shell=True, capture_output=True, text=True ).stdout ).strip().splitlines()
    bt={}
    for line in out:
        line=line.strip()
    ...
    out = str( subprocess.run([ '/bin/bluetoothctl devices'  ], shell=True, capture_output=True, text=True ).stdout ).strip().splitlines()        
    devices={}
    for line in out:
        l=line.strip().split()
        if l[0]!='Device':
    ...
    for dev in bt['paired'].keys():
        ...
        for lines in out:
            l=line.strip().split(':')
            if l[0]=='Connected' and l[1]=='yes':
                bt['connected']=dev
    return bt        

def btdiscover():
    out = str( subprocess.run([ '/bin/bluetoothctl power on'  ], shell=True, capture_output=True, text=True ).stdout )
