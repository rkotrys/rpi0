...
class rplink:
    ...
    def rfcommreset(self):
        r = str(subprocess.run([ 'ps -ef |grep "pi " |grep "/lib/systemd/systemd --user"'  ], shell=True, capture_output=True, text=True )).strip().split()
        no=1
        ...
        while len(r)>0:
            ...
            r = str(subprocess.run([ 'ps -ef |grep -e "pi " |grep " 1 " '  ], shell=True, capture_output=True, text=True )).strip().split()
            print("len {}: {}".format(no,len(r)) )
